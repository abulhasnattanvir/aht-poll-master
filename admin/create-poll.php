...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['new_poll'])) {

    // Verify the nonce
    if (!isset($_POST['mpp_poll_nonce']) || 
    !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['mpp_poll_nonce'])), 'mpp_create_poll')) {
    ...
    $bgcolor = isset($_POST['bgcolor']) ? sanitize_hex_color(wp_unslash($_POST['bgcolor'])) : '';
    ...
    echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Poll created successfully! Use the following shortcode to display the poll:', 'aht-poll-master') . ' <strong>' . esc_html($shortcode) . '</strong></p></div>';
}
